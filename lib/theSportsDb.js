...
export async function getMatchesByDate(date) {
  ...
  const raw = await apiGet(`/eventsday.php?d=${date}&l=${LEAGUE_ID}`);
  const events = (raw.events || []).filter((e) => !SEASON || e.strSeason === SEASON);
  const data = (await Promise.all(events.map(simplifyEvent))).sort(byKickoff);

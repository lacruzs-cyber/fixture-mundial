...
import { getFlagUrl, getTvChannels } from "./teamMeta";
...
export async function getAllMatches() {
  ...
  const all = (await Promise.all(events.map(simplifyEvent))).sort(byKickoff);
...
async function simplifyEvent(ev) {
  ...
  return {
    ...
    channels: getTvChannels(homeName, awayName),
    home: {
      name: homeName,
      logo: homeLogo,
